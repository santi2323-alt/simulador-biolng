...
st.set_page_config(
    page_title="Naturgy · Simulador Bio-LNG Jalisco",
    page_icon="◆",
    layout="wide",
...
st.sidebar.markdown(
    '<p style="color:#9CC3DC; font-size:12px; margin-top:-8px;">'
...
clientes_diesel = st.sidebar.slider(
    "Clientes activos que sustituyen diésel",
    0,
    5,
    3,
    1,
    help="AKRON, Lala, 2 CEDIS Walmart, Bimbo",
)
...
st.markdown(
    '<div class="ng-section"><div class="ng-section-bar"></div>'
    '<div class="ng-section-title">Indicadores clave</div>'
    '<div class="ng-section-ref">FIG. 1 · TIEMPO REAL</div></div>',
    unsafe_allow_html=True,
)
...
def gauge(value, title, vmin, vmax, color, ref=None):
    ind = dict(
        ...
        gauge={
            ...
            'steps': [
                {
                    'range': [vmin, vmin + (vmax - vmin) * 0.5],
                    'color': '#EAF1F6',
                },
                {
                    'range': [vmin + (vmax - vmin) * 0.5, vmax],
                    'color': '#DFEAF2',
                },
            ],
            'threshold': {
    ...
    if ref is not None:
        ind['delta'] = {
            'reference': ref,
            'increasing': {'color': VERDE},
            'decreasing': {'color': ROJO},
            'font': {
                'size': 12,
                'family': 'Archivo',
            },
...
with left:
    st.markdown(
        '<div class="ng-section"><div class="ng-section-bar"></div>'
        '<div class="ng-section-title">Curva de ganancia por escala</div>'
        '<div class="ng-section-ref">FIG. 2</div></div>',
        unsafe_allow_html=True,
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color=AZUL_CLARO, width=2, dash='dot'),
        )
    )
    ...
    fig.add_vline(
        ...
        annotation_font_color=NARANJA,
    )

    fig.update_layout(
        plot_bgcolor="#FFFFFF",
        paper_bgcolor="#FFFFFF",
        font=dict(color=GRIS_TX, family="Barlow", size=12),
        height=290,
    ...
    st.markdown(
        '<div class="ng-section"><div class="ng-section-bar"></div>'
        '<div class="ng-section-title">Sustitución de diésel</div>'
        '<div class="ng-section-ref">FIG. 3</div></div>',
        unsafe_allow_html=True,
    )
...
with right:
    st.markdown(
        '<div class="ng-section"><div class="ng-section-bar"></div>'
        '<div class="ng-section-title">Corredor de distribución</div>'
        '<div class="ng-section-ref">FIG. 4 · JALISCO</div></div>',
        unsafe_allow_html=True,
    )
    ...
    clientes = pd.DataFrame([
        {
            "name": "AKRON · Lagos de Moreno",
            "lat": 21.3850,
            "lon": -101.9050,
            "km": 7,
        },
        {
            "name": "Grupo Lala · Aguascalientes",
            "lat": 21.8550,
            "lon": -102.2960,
            "km": 82,
        },
        {
            "name": "Walmart CEDIS · Silao Bajío",
            "lat": 20.9480,
            "lon": -101.4280,
            "km": 92,
        },
        {
            "name": "Walmart CEDIS · Tlajomulco GDL",
            "lat": 20.4700,
            "lon": -103.4450,
            "km": 197,
        },
        {
            "name": "Grupo Bimbo · Zapopan GDL",
            "lat": 20.7060,
            "lon": -103.4530,
            "km": 198,
        },
    ...
    planta_df = pd.DataFrame([
        {
            "name": "PLANTA Bio-LNG · Lagos de Moreno",
            "lat": PLANTA["lat"],
            "lon": PLANTA["lon"],
        }
    ])
    ...
    st.pydeck_chart(
        pdk.Deck(
            ...
            layers=[layer_lines, layer_cli, layer_pl],
            tooltip={"text": "{name}"},
        ),
    ...
    if viable:
        st.markdown(
            f'<div class="badge-ok">VIABLE · {signo}{ganancia_actual:,.1f} M USD/año</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f'<div class="badge-no">NO VIABLE · ajusta precio o feed-in</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        <div class="card" style="margin-top:10px;">
            ...
            <div style="color:{AZUL}; font-weight:700; margin-top:4px; font-size:15px;">
                {ruta}
            </div>
            <div style="color:{GRIS_SUAVE}; font-size:12px; margin-top:6px; line-height:1.5;">
                {"El Bio-LNG viaja en camión hasta 198 km y sustituye diésel en industria y centros de distribución." if es_biolng else "El biometano se inyecta a la red local de Naturgy en zona Bajío Sur."}
            </div>
        </div>
        ''',
        unsafe_allow_html=True,
...
st.markdown(
    ...
    'Clientes: AKRON, Grupo Lala, Walmart CEDIS, Grupo Bimbo'
    '</p>',
    unsafe_allow_html=True,
)
