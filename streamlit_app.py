...
def gauge(value, title, vmin, vmax, color, suffix="", steps_color=None):
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=value,
        number={'suffix': suffix, 'font': {'size': 26, 'color': '#15241D', 'family': 'JetBrains Mono'}},
        title={'text': title, 'font': {'size': 11, 'color': '#7E8C86', 'family': 'JetBrains Mono'}},
        gauge={
            'axis': {'range': [vmin, vmax], 'tickcolor': '#C5CFD6', 'tickfont': {'size': 9, 'color': '#A3B1BC'}},
            'bar': {'color': color, 'thickness': 0.28},
            'bgcolor': '#F4F7F5',
            'borderwidth': 1,
            'bordercolor': '#E2E8E5',
            'steps': [
                {'range': [vmin, vmin + (vmax-vmin)*0.5], 'color': '#EEF3F0'},
                {'range': [vmin + (vmax-vmin)*0.5, vmax], 'color': '#E4EDE8'},
            ],
...
with g1:
    st.plotly_chart(gauge(biomasa, "BIOMASA t/día", 200, 1600, VERDE), use_container_width=True, key="g1")
with g2:
    st.plotly_chart(gauge(biometano_m3d, "BIOMETANO m³/día", 0, 43200, VERDE_OSC), use_container_width=True, key="g2")
with g3:
    st.plotly_chart(gauge(biolng_m3d, "BIO-LNG m³/día", 0, 14256, AZUL), use_container_width=True, key="g3")
with g4:
...
with left:
    st.markdown('<div class="section-title">Curva de ganancia según escala de biomasa</div>', unsafe_allow_html=True)
